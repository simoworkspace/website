...
export const DashboardComponent: FC = () => {
	...
	const handleBioChange = (event: React.ChangeEvent<HTMLInputElement>) =>
		setEditActions({
			...
			banner_url: editActions.banner_url,
			changesMade: true,
		});
	...
	return user ? (
		<main className="max-w-[1500px] flex justify-center items-center">
			<section className="w-screen flex flex-row p-5 text-white items-start xl:items-center justify-center gap-10 xl:flex-col">
				<div
					className={`${borderColor[color]} max-w-[250px] xl:max-w-none border-2 ${editActions.banner_url ? "min-h-[300px]" : "p-6"} w-[300px] xl:w-[90vw] rounded-lg bg-neutral-900 flex justify-start flex-col gap-4 relative`}
				>
					{editActions.banner_url && (
						<img
							className={`w-full h-36 object-cover rounded-md rounded-b-none z-0 mb-14 bg-neutral-800`}
							onError={({ currentTarget }) => {
								currentTarget.src =
									"http://www.luquips.com.br/wp-content/uploads/2015/04/banner-vazio-300x86.png";
							}}
							src={!editActions.banner_url ? "" : editActions.banner_url}
						/>
					)}
					<div
						className={`w-full items-center flex justify-center ${editActions.banner_url && "z-1 absolute top-36 left-0 transform -translate-y-1/2"}`}
					>
						<img
							onError={({ currentTarget }) => {
				...
				<div className="flex h-full w-full flex-col">
					<h1 className="text-[33px] text-center mb-5 mt-1">
						Bem vindo a dashboard, <strong>{user?.username}</strong>
					</h1>
					<div
						className={`w-full p-3 h-full flex flex-col gap-3 bg-neutral-900 ${borderColor[color]} border-2 rounded-lg`}
					>
						<Tabs position="relative" variant="unstyled">
							...
							<TabIndicator
								className={`mt[-1.5px] h-[2px] ${borderColor[color]} border-2 rounded-lg`}
							/>
							<TabPanels>
								<TabPanel>
									<div className="flex xl:flex-col gap-3 w-full py-3">
										<div className="flex flex-col items-start justify-center flex-grow h-14">
											...
											<span>
												Fale mais sobre você, digite uma breve descrição.
											</span>
										</div>
										<form className="w-[50%] xl:w-full xl:h-14 flex flex-row">
											<div className="w-full">
												<input
													defaultValue={!editActions.bio ? "" : editActions.bio}
													placeholder="Digite sua biografia aqui."
													maxLength={200}
													value={!editActions.bio ? "" : editActions.bio}
													required
													onChange={handleBioChange}
													className={`bg-transparent disabled:opacity-50 focus:outline-none border-2 rounded-lg p-2 w-full h-14 ${borderColor[color]}`}
													type="text"
												/>
											</div>
										</form>
									...
									<div className="flex xl:flex-col gap-3 w-full py-3">
										<div className="flex flex-col items-start justify-center flex-grow h-14">
											...
											<span>
												Coloque o link de uma imagem para ser seu banner.
											</span>
										</div>
										<form className="w-[50%] xl:w-full xl:h-14 flex flex-row">
											<div className="w-full">
												<input
													...
													placeholder="Coloque uma URL de uma imagem aqui."
													maxLength={200}
													value={
														!editActions.banner_url
															? ""
			...
			{editActions.changesMade && (
				<div
					className={`${editActions.changesMade ? "bounceIn" : "invisible"} w-[90vw] absolute xl:z-10 xl:fixed xl:bottom-10 xl:w-[95vw] bottom-5 bg-neutral-800 ${borderColor[color]} border-2 rounded-lg duration-200`}
				>
					<div className="flex p-2 text-white w-full items-center">
						<span className="flex flex-grow">
							Você tem alterações para serem salvas
						</span>
						<div className="flex gap-2 items-center">
							...
							>
								{!editActions.changesLoading ? (
									...
									<iconAI.AiOutlineLoading3Quarters
										fill="#fff"
										size={22}
