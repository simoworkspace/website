...
export const DashboardUser: FC<{
    ...
    return (
        <div className={`${borderColor[color]} border-2 w-[300px] py-8 xl:w-[90vw] rounded-lg bg-neutral-900 flex items-center justify-center flex-col`}>
            ...
            <div className="mt-8">
                {!page ? <Button clas="text-center flex items-center gap-1" link to={`/dashboard/settings`}><icon.BsGear/>Configurações</Button> : <Button clas="text-center flex items-center gap-1" link to={`/dashboard`}><icon.BsBack/>Voltar</Button>}
            </div>
        </div>
    )
};