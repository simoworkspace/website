...
export const BotComponent: FC = () => {
	...
	const getBotStars = async () => {
		...
		if (!stars.length) return setStars(0);
	...
	const getBotData = async () => {
		try {
			const {
				data: { owner_id },
				data,
			} = await api.getBotInfos(botid);

			const {
				data: { username, avatar, id },
			} = await api.getUserFromDB(owner_id);
		...
		<section className="max-w-[1500px] w-screen">
			{!botData.approved && (
				<div className="fixed flex items-center justify-center backdrop-blur-sm inset-0">
					<div className="flex gap-3 items-center justify-center flex-col w-full h-[150px] border-2 rounded-lg bg-[#e8a60c] border-[#9e7514]">
						...
						<span className="text-lg text-center px-2">
							Bot {botData.name} está em análise, aguarde até que ela seja
							finalizada.
						</span>
					</div>
				</div>
			...
			<div className="flex flex-col items-center justify-center">
				<section className="flex items-center xl:flex-col justify-center w-full xl:mt-2 mt-[30px] text-white">
					<div
						className={`bg-neutral-900 rounded-xl flex xl:flex-col xl:h-[320px] h-[120px] w-[95%] border-2 ${borderColor[color]} items-center justify-center`}
					>
						<img
							...
							onError={async ({ currentTarget }) => {
								...
								currentTarget.src = (
									await import("../../assets/images/simo.png")
								).default;
							}}
				...
				<section
					className={`w-[90%] mb-5 bg-neutral-900 border-2 ${borderColor[color]} border-t-0 rounded-t-none rounded-lg p-10 xl:p-3`}
				>
							...
							<div className="w-full">
								<div className="w-full">
									...
									<div className="grid grid-cols-2 gap-4">
										<Link
											to={`/user/${dev?.id}`}
											className="border-2 border-neutral-800 p-2 rounded-lg flex flex-row flex-wrap justify-center xl:flex-col items-center gap-4 transition-colors duration-300 hover:bg-neutral-800"
										>
											<img
												onError={async ({ currentTarget }) => {
													currentTarget.onerror = null;
							...
							<div>
								...
								<div className="flex flex-col w-full gap-3">
									<div>
										...
										<span>{botData.prefixes.join(", ")}</span>
									</div>
									<div>
										<strong className="text-lg">Votos </strong>
										<span>
											{botData.votes.reduce(
												(votesCount, vote) => votesCount + vote.votes,
												0,
											)}
										</span>
									</div>
									<div>
										<div className="flex flex-row gap-3 flex-wrap">
											...
											{botData.tags.map((tag, index) => (
												<div
													key={index}
													className={`${borderAndBg[color]} p-[6px] rounded-lg border-2`}
												>
													{tag}
												</div>
											))}
										...
										<>
											...
											<div className="flex flex-col gap-3 flex-wrap">
												{botData?.support_server && (
													<Link
														to={
															botData?.support_server.includes("https://")
																? botData?.support_server
																: "https://" + botData?.support_server
														}
												...
												{botData?.source_code && (
													<Link
														to={
															botData?.source_code.includes("https://")
																? botData?.source_code
																: "https://" + botData?.source_code
														}
														className="flex items-center gap-3 p-2"
													>
												...
												{botData?.website_url && (
													<Link
														to={
															botData?.website_url.includes("https://")
																? botData?.website_url
																: "https://" + botData?.website_url
														}
														className="flex items-center gap-3 p-2"
													>
